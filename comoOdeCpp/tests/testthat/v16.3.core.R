...
# file_path <- paste0(getwd(),"/Testtemplate3.xlsx")  
...
dta <- (read_excel(file_path, sheet = "School Closure",col_names = TRUE ))
dta<-as.data.frame(dta) 
closure_group<-cbind(((dta[,2:5])),matrix(0,nrow = 15,17))
    ...
    for (i in 1:length(sc)){
      ...
      if(i==1){
        if (inp$`Date Start`[sc[i]]>startdate){
          ...
          schoolclose<-c(rep(0,f[i]*20),rep(inp$`Value`[scc[i]],(f[i+1]-f[i])*20))
        }
        else{
          sc_vector<-c(rep(inp$`Value`[sc[i]],(f[i+1])*20))
          schoolclose<-c(rep(inp$`Value`[scc[i]],(f[i+1])*20))
        }
      }
      else{
        if (f[(i-1)*2+1]-f[(i-1)*2]==1){
          sc_vector<-c(sc_vector,rep(inp$`Value`[sc[i]],20))
          schoolclose<-c(schoolclose,rep(inp$`Value`[scc[i]],20))
        }else{
        ...
        schoolclose<-c(schoolclose,rep(inp$`Value`[scc[i]],(f[i*2]-f[i*2-1])*20))
      }
...
critV <- round(critV * to_keep) / to_keep
         ...
         if (schoolclose>=1){
           school<-school_eff
           if(schoolclose==1){schoolclose2<-1}
           else{
            schoolclose2<-as.numeric(closure_group[schoolclose,])
         print("schoolclose2")
         print(schoolclose2)
          }
         }else{schoolclose2<-0}
         ...
         # contact matrices
         if(length(schoolclose2)>1){
         print("contact_school")
         print(contact_school)
